...
Nette\Config\Configurator;self::$configurator->setCacheDirectory(defined('TEMP_DIR')?TEMP_DIR:ini_get('upload_tmp_dir'));self::$createdAt='?';foreach(debug_backtrace(FALSE)as$row){if(isset($row['file'])&&is_file($row['file'])&&strpos($row['file'],NETTE_DIR.DIRECTORY_SEPARATOR)!==0){self::$createdAt="$row[file]:$row[line]";break;}}}return
...
class
Framework{const
NAME='Nette Framework',VERSION='2.0-beta',REVISION='325620c released on 2011-12-19';public
...
function
parseService(Nette\DI\ServiceDefinition$definition,$config,$shared=TRUE){if(!is_array($config)){$config=array('class'=>$config);}$known=$shared?array('class','factory','arguments','setup','autowired','run','tags'):array('class','factory','arguments','setup','autowired','internal','parameters');if($error=array_diff(array_keys($config),$known)){throw
new
